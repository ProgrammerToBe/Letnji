...
import korenski.repository.geografija.KlijentRepository;
import korenski.repository.geografija.NaseljenoMestoRepository;
import korenski.repository.geografija.PravnoLiceRepository;
...
public class PravnoLiceController {
	...
	@RequestMapping(
			...
			method = RequestMethod.GET,
			produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<Collection<PravnoLice>> nadjiKlijente(@PathVariable("id") Long id) throws Exception {
	...
	public ResponseEntity<Collection<PravnoLice>> filtrirajPravnaLicaZaNaseljenoMesto(@PathVariable("jmbg") String jmbg,
		...
		return new ResponseEntity<Collection<PravnoLice>>( repository.(jmbg, ime, prezime, adresa, telefon, email, pib, fax, odobrio, naseljenoMesto), HttpStatus.OK);
	}
}
